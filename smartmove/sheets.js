// api/sheets.js — reads public Google Sheet via CSV export
// No API key or service account needed — sheet is "Anyone with link"
...
export default async function handler(req, res) {
  ...
  try {
    if (action === "getAllTeamTabs") {
      // Fetch main sheet as public CSV
    ...
    if (action === "getSourceTabs") {
      ...
      try {
        ...
        if (r.ok) { const { rows } = parseCSV(await r.text()); results["XELM"] = rows; }
        else results["XELM"] = [];
